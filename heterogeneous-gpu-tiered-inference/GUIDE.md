# Mixed-GPU Cluster Implementation Guide
...
flowchart TB
    ...
    RF_H200["ResourceFlavor<br/>gpu-h200<br/><i>nodeLabels: H200-141GB-HBM3e</i>"]
    ...
    style RF_H200 fill:#bf360c,stroke:#ff7043,color:#fff
...
metadata:
  name: a100-inference
spec:
  cohort: gpu-shared-pool          # enables cross-tier borrowing if desired
  namespaceSelector: {}
  resourceGroups:
...
| Tensor parallelism | **Supported (GA)** | vLLM `--tensor-parallel-size` + multi-GPU pod request |
| Topology-optimized TP | **Gap** | TAS not in RHBoK/RHOAI; non-issue on H100 SXM |
| Intelligent request routing | **Supported (GA)** | llm-d EPP with queue-scorer + prefix-cache-scorer |
| Inference autoscaling | **Developer Preview** | WVA with KV cache + queue depth metrics |
